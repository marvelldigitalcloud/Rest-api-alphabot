...
const listkey = ["Nasabot", "marvellganz"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Nasabot') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Nasabot')  return res.json(loghandler.invalidKey)
...
router.get('/canvas/welcome3', async (req, res, next) => {
  mim = req.query
  if (!mim.pp) return res.json({ status: 404, msg: 'Masukkan Param pp'})
  if (!mim.ppgc) return res.json({ status: 404, msg: 'Masukkan Param ppgc'})
  if (!mim.name) return res.json({ status: 404, msg: 'Masukkan Param name'})
  if (!mim.bg) return res.json({ status: 404, msg: 'Masukkan Param bg'})
  if (!mim.grupname) return res.json({ status: 404, msg: 'Masukkan Param grupname'})
  if (!mim.member) return res.json({ status: 404, msg: 'Masukkan Param member'})
  try {
    const knights = require("knights-canvas")
    var image = await new knights.Welcome()
    .setUsername(mim.name)
    .setGuildName(mim.grupname)
    .setGuildIcon(mim.ppgc)
    .setMemberCount(mim.member)
    .setAvatar(mim.pp)
    .setBackground(mim.bg)
    .toAttachment()
    ...
    res.json({ status: 404, msg: 'EMROR 404 :v'})
  }
